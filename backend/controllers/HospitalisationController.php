...
class HospitalisationController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $hospitaliser->load(Yii::$app->request->post()) ) {
            ...
            $infochbre = Chambre::find()
                ...
                ->one();
            $infopat= Hospitaliser::find()
                ->where(['idpatient' => $_POST['Hospitaliser']['idpatient']])
                ->one();

           // $hospitaliser->tauxassurance = $infochbre->getTaux($hospitaliser->idpatient0->idassurance);
            $hospitaliser->tauxassurance = $hospitaliser->idpatient0->tauxassu;
            $tauAss=1-$hospitaliser->idpatient0->tauxassu/100;


            $hospitaliser->coutunitchbre = $infochbre->coutchbre * $tauAss;
            $hospitaliser->	coutbrut = $infochbre->coutchbre ;
            $hospitaliser->	coutassurance = $infochbre->coutchbre *$hospitaliser->idpatient0->tauxassu/100;
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $hospitaliser->load(Yii::$app->request->post()) ) {
            ...
            //var_dump($hospitaliser->coutunitchbre);exit;
            if($model->save() && $hospitaliser->save()) {
    ...
    public function actionPrint($idhospitalisation, $idpatient )
    {
        return $this->renderPartial('print', [
            'model' => $this->findModel($idhospitalisation, $idpatient ),
        ]);
    }
}
